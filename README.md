...
| `pdf_save [URL] [PATH]` | `{"action":"pdf_save","args":["URL","PATH"]}` | 保存PDF到本地（绕过Chrome内置PDF查看器） |
...
### v2.1.5 (2026-06-13)
- **新命令**：`md` — 页面转 Markdown（保留标题/链接/列表/图片结构）
- **新功能**：结构化提取 — 用 `container` + `fields` 选择器批量提取 JSON
  - 支持 `@attr` 语法提取属性值（`h2 a @href`）
  - Do 链原生支持（container/fields/limit/output 作为步骤字段）
- **新功能**：浏览记录 — 每次 `goto` 自动写入 `logs/YYYY-MM-DD/history.md`
- **新功能**：条件等待 — `wait` 支持 text/js/network_idle/title/url 五种条件
  - Do 链: `{"action":"wait","args":[{"kind":"text","value":"加载完成"}]}`
  - CLI: `py agent_browser.py wait --text "登录成功"`

### v2.1.4 (2026-06-11)
- **新功能**：do 链错误恢复 — `on_error: stop/skip/retry/fallback` 四种策略
- **修复**：click/dblclick/wait 的 arg 为整数时 `isdigit()` 崩溃 → 加 `str()` 兜底

### v2.1.1 (2026-06-11)
- **新命令**：`pdf_save` — 绕过 Chrome 内置 PDF 查看器，将 PDF 下载到本地
  - 实现：`FileReader.readAsDataURL` 浏览器原生 base64，零字节损坏
  - 实战验证：LMO → EAP045 Module Handbook（766KB，SSO 直链成功保存）
- **修复**：pdf_save v1 用 `btoa` + `TextDecoder` 对大 PDF 不稳定，改为 `FileReader` 方案
...
- **优化**：下载路径统一到 skill 根目录（v2.1.1）
- **优化**：下载路径统一到 `skills/agent-browser/downloads/`（v2.1.1）
- **新增**：截图无名称时自动提取域名作为标识
